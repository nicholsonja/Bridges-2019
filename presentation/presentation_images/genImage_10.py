...
def makeImage(height, width, imageName):
    ...
    largeRadius  = imageWidth/2 * .95
    smallRadius = largeRadius * .75


    for i in range(numSamples):
        theta = uniform(0, 2 * pi)
        
        pA_x, pA_y = circle(kA * theta, largeRadius, cx, cy)
        pB_x, pB_y = circle(kB * theta, smallRadius, cx, cy)
    ...
    saveImage(data, imageName, imageWidth, imageHeight,
              bg=[255, 255, 255], fg=[128, 0, 128], alphaMultiplier=8)
